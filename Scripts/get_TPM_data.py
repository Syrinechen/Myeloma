...
def Correspondances_interne_id():
    d={}
    with open("/home/irit/Documents/Myeloma/Utilities/nums_internes_cassioppe.tsv") as f:
        for line in f:
...
def get_data():
    ...
    - count_list: list of gene expression values
    - responses: list of patient responses
    - rates: list of patient rates
    - res_rates: list of response classes from rates
    """
    directory = '/home/irit/Documents/Myeloma_Syrine/Data/TPM_data(old)'
    ...
    for path in tqdm(pathlist):
        patient_id = str(path).split('/')[7].split('_')[0]
        if patient_id[0] == 'T':
            if patient_id not in dict_interne_id.keys():
                print(patient_id, ' not found')
            else:
                patient_id = dict_interne_id[patient_id]
        genes_counts=pd.read_csv(str(path),sep='\t')
        count_list.append(genes_counts[genes_counts.columns[-1]].values.tolist())   
...
# %% execute this to get the files tpm_count_mrd_response(old).csv and tpm_count_mrd_rate(old).csv
if __name__ == "__main__":
    ...
    data_classif.to_csv('tpm_count_mrd_response.csv',index_label='patient_id')
    data_reg.to_csv('tpm_count_mrd_values.csv',index_label='patient_id')    
# %%
